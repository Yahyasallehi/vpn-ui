...
![Общий вид](https://raw.githubusercontent.com/Yahyasallehi/vpn-ui/refs/heads/main/media/overview.png)
...
- Индивидуальные **лимиты скорости** для каждого аккаунта (раздельные ограничения загрузки и отдачи), применяемые непосредственно в пропатченном ядре **Xray-core** и обновляемые «на лету» (hot-reload) без перезапуска панели и разрыва активных соединений
- **Лимиты подключений/устройств**, применяемые ядром на этапе установления соединения, без зависимости от внешнего jail **fail2ban**
- Аккаунты, охватывающие сразу несколько **Inbound** и протоколов в рамках одной общей квоты, одного срока действия и одной ссылки подписки, вместо отдельного клиента на каждый Inbound
- **Tunnel Carrier**: маршрутизация внешнего соединения любого VPN-протокола через любой **Outbound** Xray (WARP, другой прокси и т. д.), а не только через другой VPN-туннель
...
![Тест E2E](https://raw.githubusercontent.com/Yahyasallehi/vpn-ui/refs/heads/main/media/test_unit.png)
