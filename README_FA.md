...
<p align="center">
  <img src="https://raw.githubusercontent.com/Yahyasallehi/vpn-ui/refs/heads/main/media/logo.png" alt="VPN-UI Logo" width="260">
</p>
...
![نمای کلی](https://raw.githubusercontent.com/Yahyasallehi/vpn-ui/refs/heads/main/media/overview.png)
...
- هسته‌ی [**Xray-core** پچ‌شده](https://github.com/Sir-MmD/Xray-core) برای رفع خطای «Unsupported Cipher» در پروتکل **Shadowsocks**، و اضافه‌شدن **AnyTLS**، **TUIC** و **NaiveProxy** به‌عنوان پروتکل‌های Native، طوری که مثل بقیه‌ی پروتکل‌ها از حسابداری ترافیک به ازای هر اکانت، محدودیت سرعت، محدودیت دستگاه و تشخیص آنلاین‌بودن بهره می‌برن، بدون نیاز به یک هسته‌ی دوم
- **محدودیت سرعت جداگانه** برای هر اکانت (سقف مجزا برای آپلود و دانلود)، که مستقیماً داخل هسته‌ی Xray-core پچ‌شده اعمال می‌شه و به‌صورت آنی (Hot-Reload) بارگذاری می‌شه، بدون نیاز به ریستارت پنل یا قطع اتصال‌های زنده
- **محدودیت اتصال/دستگاه** که در همون لحظه‌ی برقراری اتصال، داخل خودِ هسته اعمال می‌شه؛ بدون وابستگی به jail بیرونیِ fail2ban
- اکانت‌هایی که روی **چند Inbound/پروتکل** مختلف، با یک حجم مشترک، یک تاریخ انقضا و یک لینک ساب واحد کار می‌کنن، به‌جای اینکه برای هر Inbound یک کلاینت جداگانه لازم باشه
- قابلیت **Tunnel Carrier**: مسیر کردن اتصال بیرونیِ هر پروتکل VPN از طریق هر Outbound دلخواه Xray (مثل WARP یا یک پراکسی دیگه)، نه فقط از طریق یک تونل VPN دیگه
...
```bash
curl -Ls https://raw.githubusercontent.com/Yahyasallehi/vpn-ui/refs/heads/main/deploy.sh | sudo bash
```
...
```bash
git clone https://github.com/Yahyasallehi/vpn-ui.git && cd vpn-ui
./build.sh
```

## تست E2E

![تست E2E](https://raw.githubusercontent.com/Yahyasallehi/vpn-ui/refs/heads/main/media/test_unit.png)
